 ...
 * 
 * 修改记录：
 * 2013.1.15： modified by xiaoqing.qiu. 同一个window下弹出多个弹出框，只有当关闭所有的弹出框后才隐藏遮罩层，否则改变遮罩的z-index。
 * 2013.2.4: 下一个弹窗z-index置顶
 * 2013.2.4: 窗口resize 判断修改
 */
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	unlock: function()
	{
		...
		/***
		 * 同一个window下弹出多个弹出框，只有当关闭所有的弹出框后才隐藏遮罩层，否则改变遮罩的z-index
		 * modified by xiaoqing.qiu. 2013.1.15
		 * update 2013-2-4
		 * .DOM.wrap弹窗Dom
		 */		
				var zIndex = 0, list = lhgdialog.list;
				for(var d in lhgdialog.list){
					if(lhgdialog.list[d] !=that && lhgdialog.list[d]._lock && !$(lhgdialog.list[d].DOM.wrap).is(":hidden")){
						zIndex = Math.max(zIndex, lhgdialog.list[d].DOM.wrap.css("z-index"));
					}
				}
				if(zIndex>0){
					mask.style.zIndex = parseInt(zIndex,10) - 1;
				}else{
					mask.style.display = 'none';
				}
	//			mask.style.display = 'none';	//modified by xiaoqing.qiu. 2013.1.15
			}
	...
	close: function()
	{
		...
		/***
		 * 下一个弹窗z-index置顶
		 * modified by xiaoqing.qiu. 2013.2.4
		 */
		var focusDiv=null, zIndex=0;
		for(var d in lhgdialog.list){
			if(lhgdialog.list[d] !=that && !$(lhgdialog.list[d].DOM.wrap).is(":hidden")){
				if(zIndex < lhgdialog.list[d].DOM.wrap.css("z-index")){
					zIndex = lhgdialog.list[d].DOM.wrap.css("z-index");
					focusDiv = lhgdialog.list[d];
				}
			}
		}
		if(focusDiv){
